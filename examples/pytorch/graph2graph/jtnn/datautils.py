...
class Graph2GraphDataset(Dataset):
    def __init__(self, data, vocab, training=True, exp='drd2', mode='pair'):
        ...
        self.dir = os.getcwd()
        print('Loading data...')
        data_file = '{}/data/{}/{}.txt'.format(self.dir, exp, data)
        self.mode = mode
        ...
        self.vocab_file = '{}/data/{}/{}.txt'.format(self.dir, exp, vocab)
    ...
    def __getitem__(self, idx):
        if self.mode == "pair":
            return [self._get_single(self.data[idx][0]), self._get_single(self.data[idx][1])]
        else:
            return self._get_single(self.data[idx])
    def _get_single(self, example):
        smiles = example
...
class Graph2GraphCollator(object):
    ...
    def __call__(self, examples):
        if self.mode == 'pair':
            #print("collator get called!")
            examples_in_graphs, examples_out_graphs = list(zip(*examples))
            return [self._call_single(examples_in_graphs), self._call_single(examples_out_graphs)]
        else:
            return self._call_single(examples)

    def _call_single(self, examples):
        # get list of trees
